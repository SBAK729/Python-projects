...
def add():
    ...
    date = get_date("Enter the date of treansaction (dd-mm-yyyy) or press Enter for Today's date: ", allow_default=True)
    amount = get_amount()
...
def plot_transaction(df):
    ...
    income_df = (df[df["category"] == "Income"].resample("D")["amount"].sum().reindex(df.index, fill_value=0))

    expence_df = (df[df["category"] == "Expence"].resample("D")["amount"].sum().reindex(df.index, fill_value=0))

    plt.figure(figsize=(10, 5))
    plt.plot(income_df.index, income_df.values, label= "Income", color="g")
    plt.plot(expence_df.index, expence_df.values, label= "Expence", color="r")
    plt.xlabel("Date")
    plt.ylabel("Amount")
